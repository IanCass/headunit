...
public class StreamVideoExtractor implements MediaExtractorInterface {
    ...
    public int readSampleData(ByteBuffer buffer, int offset) {
        if (mSampleOffset >= 0) {
            ...
            AppLog.d("readSampleData (offset: %d,next: %d,size: %d,length: %d, flags: 0x%08x)",mSampleOffset, nextSample, size, mContentData.length, mFlags);
            return size;
        }
        return 0;
    }
    ...
    private int findNextNAL(int offset) {
        while(offset < mContentData.length) {
            if (mContentData[offset] == 0 && mContentData[offset + 1] == 0 && mContentData[offset + 2] == 0 && mContentData[offset + 3] == 1) {
                AppLog.d("Found sequence at %d: 0x0 0x0 0x0 0x1 0x%01x (%d)",mSampleOffset, mContentData[offset + 4], mContentData[offset + 4]);
                return offset;
            }
            offset++;
        }
